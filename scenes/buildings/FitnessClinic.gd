...
func _build_ui() -> void:
	...
	var building = GameState.campus_buildings.get("Fitness Clinic", {})
	var lbl_title = Label.new()
	lbl_title.text = "💪 FITNESS CLINIC  ·  Level %d" % building.get("level", 1)
	...
	left.add_child(_section_label("DRIVER FITNESS"))
...
func _build_extra_panel() -> PanelContainer:
	...
	if drivers.is_empty():
		...
		for d in drivers:
			...
			lbl_name.custom_minimum_size.x = 150
			lbl_name.add_theme_font_size_override("font_size", 24)
			row.add_child(lbl_name)
			var bar = ProgressBar.new()
			bar.min_value = 0; bar.max_value = 100; bar.value = d.fitness
			bar.custom_minimum_size = Vector2(120, 14); bar.show_percentage = false
			var fc = Color(0.3,0.9,0.4) if d.fitness >= 70 else (Color(1.0,0.75,0.1) if d.fitness >= 40 else Color(1.0,0.3,0.3))
			bar.modulate = fc
			row.add_child(bar)
			var lbl_fit = Label.new()
			lbl_fit.text = "%.0f%%" % d.fitness
			lbl_fit.modulate = fc
			lbl_fit.add_theme_font_size_override("font_size", 24)
			row.add_child(lbl_fit)
			vbox.add_child(row)
	return panel
