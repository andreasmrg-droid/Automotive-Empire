...
func _build_ui() -> void:
	...
	lbl_title.text = "🔬 R&D DESIGN STUDIO  ·  Level %d" % building.get("level", 1)
	...
	lbl_desc.text = "Houses Designers who develop blueprints for car parts. Unlocks R&D projects and reverse engineering."
	...
	left.add_child(_section_label("DESIGNERS"))
...
func _build_extra_panel() -> PanelContainer:
	...
	if designers.is_empty():
		...
		for d in designers:
			...
			row.add_child(ln)
			var ls = Label.new()
			ls.text = "CR %d/wk" % int(d.weekly_salary)
			ls.modulate = Color(0.6, 0.6, 0.6)
			ls.add_theme_font_size_override("font_size", 12)
			row.add_child(ls)
			vbox.add_child(row)
	return panel
